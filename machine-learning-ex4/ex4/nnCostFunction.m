...
J = (-1/m) * (sum(sum(d.*log(a3)) + sum((1-d).*log(1-a3))));
...
for i=1:m,
	...
	y3i = d(i, :);
	del3i = a3i - y3i;
	del2i = (Theta2)'*del3i'.*sigmoidGradient([1;Theta1*a1i']);
	del1 = del1 + del2i(2:end)*a1i;
	del2 = del2 + del3i' * a2i;
end;
dumTheta1 = [zeros(size(Theta1, 1), 1) Theta1(:, 2:end)];
dumTheta2 = [zeros(size(Theta2, 1), 1) Theta2(:, 2:end)];
error = (lambda/(2*m)) * (sum(sum(dumTheta1.^2)) + sum(sum(dumTheta2.^2)));
J = J + error; %Regularized cost function
